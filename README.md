...
> 单机六系统部署(国内网络):`docs/deployment_single_host.md` + `deploy/bootstrap.sh`。
> 交付验收总览见 `docs/DELIVERY_REPORT.md`(测试矩阵/GAP 终态/缺陷台账/一键运行指引)。
> 本仓库当前处于 **里程碑 10 已交付**(国密套件真实现 + 套件迁移体系 + 三包流水线 + 部署包)状态——全部 10 个里程碑完成。
...
- 里程碑 9(前端三形态)✅:ui-kit 内部包(设计变量/统一 fetch 四态
  文案/自研 history 路由/成对横幅/打码/同构设置页,纯函数 node 直测);
  F2 四 SPA(quiz 六页/certvault 六页 JWT 内存特例/nvr 七页/adapter 三页,
  React18+Vite+TS strict,rp_common/spa 统一托管:深链兜底+CSP+越界防护);
  F3 三维大屏(three@0.160.1 本地副本+零 addons 轨道相机+低多边形成景+
  WS 帧驱动+降级阶梯承接,nonce CSP,GAP-16 解除);F1 安全头固化
  (/admin 零 JS CSP);I 组固化(E3 双条款/外链扫描进 CI 六步,I 组
  10 项+浏览器 15 项)。249 项回归 + CI 门禁全绿。
- 里程碑 10(交付收口)✅:国密 gm 套件纯 Python 参考实现(SM3/SM4-GCM/
  SM2 标准向量+AES-GCM 对拍锚定,GAP-01 解除)、JWKS 双钥与 SM2SM3 令牌、
  13-R-IDP-2 迁移体系(双写窗口/断点续迁/审计三锚点/主密钥轮换=迁移)、
  F 组 11 用例、B2/B4/B7 基线补齐(B1-B10 全覆盖,环境指纹落盘)、
  H10 三包流水线(六系统×软著/论文/专利包)、部署包(Dockerfile/加固/
...
| 9 | 前端三形态(F1/F2/F3)+ ui-kit + Playwright E2E | I | ✅ 已交付 |
| 10 | 套件迁移脚本(双写/断点,R-IDP-2)、gm 冒烟、三包流水线、部署包 | F、交付物清单 | ✅ 已交付 |
