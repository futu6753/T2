...
"""
@file    tokens.py
@brief   OIDC 令牌铸造(IdP 侧):RS256 JWT 编码(id_token / logout_token)。
         验签在 RP 侧库 gd_sso_client.jwt_verify(按 JWKS kid 选钥)。
@author  港电实验室平台组
@date    2026-07-18
...
def encode_jwt(claims: dict, key_store: ServerKeyStore) -> str:
    """
    @brief  RS256 JWT 编码,header 携带 kid(存量兼容与双套件切换依据)
    @param  claims    载荷声明
    @param  key_store 服务器签名密钥仓
    @return JWT 字符串
    """
    header = {"alg": "RS256", "typ": "JWT", "kid": key_store.kid}
...
def mint_id_token(issuer: str, client_id: str, user: dict, groups: list,
    ...
    return encode_jwt(claims, key_store)


def mint_logout_token(issuer: str, client_id: str, account: str,
                      key_store: ServerKeyStore, now: float = None) -> str:
