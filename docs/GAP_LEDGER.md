...
| GAP-15 | Playwright/Chromium 浏览器为 E2E 依赖,离线目标环境若未预装则 `tests/e2e/*` 自动跳过(skipUnless),不阻塞门禁 | `tests/e2e/test_e2e_idp.py`、`tests/e2e/test_e2e_f3d.py` | 目标环境 `pip install playwright && playwright install chromium`(或离线包内置浏览器二进制)后浏览器用例全跑 | **已解除(里程碑 9,联网打包机)**:15 项浏览器用例实跑全绿(idp 6 + f3d 5 + frontend 4);离线目标环境仍自动跳过不阻塞 |
